...
import {CREATE_BASED_CLIENT_ORDER, markedForDeletion, statusOptions} from "../../../composables/constant/items.js";
...
const getClientOrderData = async ({
  page,
  itemsPerPage,
  sortBy,
  search,
  ...
  loading.value = true;
  try {
    const { data } = await clientOrderApi.get(
    ...
    clientOrders.value = data.result.data;
    loading.value = false;
  } catch (e) {}
};
...
const massDel = async () => {
  try {
    const { status } = await clientOrderApi.remove({ ids: markedID.value });
    if (status === 200) {
...
const massRestore = async () => {
  try {
    const { status } = await clientOrderApi.restore({ ids: markedID.value });
    if (status === 200) {
...
const lineMarking = (item) => {
  if (markedID.value.length > 0) {
    const firstMarkedItem = clientOrders.value.find(
...
const approve = async () => {
  try {
    const res = await clientOrderApi.approve({ ids: markedID.value });
...
const unApprove = async () => {
  try {
    await clientOrderApi.unApprove({ ids: markedID.value });
    showToast(approveDocument);
    await getClientOrderData({});
    markedID.value = [];
  } catch (e) {
...
watch(markedID, (newVal) => {
  markedItem.value = clientOrders.value.find((el) => el.id === newVal[0]);
});
...
const show = (item) => {
  window.open(`/clientOrder/${item.id}`, "_blank");
};
      ...
      <div class="d-flex justify-end ga-2">
        <div class="d-flex w-100 justify-end mb-3">
          <div class="d-flex ga-2 position-relative">
            ...
            />
            <create-base :marked-i-d="markedID[0]" :items="CREATE_BASED_CLIENT_ORDER" />
          </div>
        </div>
