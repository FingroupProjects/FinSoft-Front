...
const getClientReturnData = async ({ page, itemsPerPage, sortBy, search }) => {
  ...
  loading.value = true;
  try {
    const { data } = await clientReturnApi.get(
...
const unApprove = async () => {
  try {
    await clientReturnApi.unApprove({ ids: markedID.value });
    showToast(approveDocument);
    await getClientReturnData({});
    markedID.value = [];
  } catch (e) {
...
const lineMarking = (item) => {
  if (markedID.value.length > 0) {
    const firstMarkedItem = clientReturns.value.find(
