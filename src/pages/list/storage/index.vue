...
const getGroups = async ({ page, itemsPerPage, sortBy, search } = {}) => {
  ...
  loading.value = true;
  try {
    const { data } = await storageGroup.get(
...
const getStorageEmployeeData = async ({
  page,
  itemsPerPage,
  sortBy,
  search,
  ...
  if (
    (idStorage.value === 0 && idStorage.value !== undefined) ||
  ...
  try {
    const { data } = await storage.getStorageEmployee(
      { page, itemsPerPage, sortBy },
      search,
      idStorage.value
    );
...
const massDelEmployee = async () => {
  try {
    const { status } = await storage.massDeletionEmployee({
...
const massRestoreEmployee = async () => {
  try {
    const { status } = await storage.massRestoreEmployee({
      ids: markedEmployeeID.value,
    });
...
const updateEmployee = async () => {
  ...
  try {
    const { status } = await storage.updateEmployee(
      idStorageEmployee.value,
      body
    );
    if (status === 200) {
      await getStorageEmployeeData();
...
const removeStorageEmployee = () => {
  if (markedEmployeeID.value.length === 0)
    return showToast(warningMessage, "warning");
...
const lineMarking = (item) => {
  if (markedID.value.length > 0) {
    const firstMarkedItem = storages.value.find(
...
const employeeLineMarking = (item) => {
  if (markedEmployeeID.value.length > 0) {
    const firstMarkedItem = storageData.value.find(
      (el) => el.id === markedEmployeeID.value[0]
    );
    if (firstMarkedItem && firstMarkedItem.deleted_at) {
      if (item.deleted_at === null) {
        showToast(ErrorSelectMessage, "warning");
...
const closingWithSaving = async () => {
  if (isExistsStorage.value) {
    ...
    const isValid = validate(nameRef, organization, group);
    showModal.value = false;
    if (isValid === true) {
      await addStorage();
      dialog.value = false;
...
const updateGroup = async () => {
  ...
  try {
    const res = await storageGroup.update(group.value.id, {
...
const isGroupChecked = (item) => {
  return item.items[0].raw.storages.every((item) =>
...
watch(dataDialog, (newVal) => {
  if (!newVal) {
    ...
    markedEmployeeID.value = [
      markedEmployeeID.value[markedEmployeeID.value.length - 1],
    ];
  }
});
  ...
  <div class="pa-4">
        ...
        <div class="d-flex justify-end ga-2">
          <div class="d-flex ga-2 mb-3">
            ...
            <Button
              name="copy"
              v-if="createAccess('storage')"
              @click="addBasedOnStorage"
            />
            <Button
              name="delete"
              v-if="removeAccess('storage')"
              @click="compute"
            />
              ...
              <td style="width: 350px">
                <div class="d-flex align-center">
                  <CustomCheckbox
                    v-if="
                      isGroupOpen(item) &&
                      item.items[0].raw.storages.length !== 0
                    "
                    v-model="markedID"
    ...
    <v-card>
      <v-dialog
        ...
        class="mt-2 pa-2"
        v-model="dialog"
        @keyup.esc="isExistsStorage ? checkUpdate() : checkAndClose()"
      >
        <v-card
        ...
        >
          <div class="d-flex justify-space-between align-center mb-2">
            <span>{{
              isExistsStorage ? "Склад: " + storageInDialogTitle : "Добавление"
            }}</span>
            <div class="d-flex align-center justify-space-between">
              <div class="d-flex ga-3 align-center mt-2">
                <Icons
                  v-if="removeAccess('storage') && isExistsStorage"
                  @click="compute"
                  name="delete"
                />
                <Icons
                ...
                />
                <Icons
                  v-if="updateAccess('storage') && isExistsStorage"
                  @click="update"
                  name="save"
                />
                <Icons
          ...
          <v-form
            class="d-flex w-100"
            :disabled="!updateAccess('storage') && isExistsStorage"
          ...
          <v-card class="table" :style="`border: 2px solid ${BASE_COLOR}`">
            <div v-if="isExistsStorage" class="d-flex w-100 rounded-t-lg mb-1 align-center" :style="`border-bottom: 2px solid ${BASE_COLOR}`" >
              <div class="d-flex justify-end w-100 ga-2 pt-1 me-2" style="padding: 4px 0px 6px 0px !important" >
                <Icons
            ...
            >
              ...
              <template v-slot:item="{ item, index }">
                <tr
                  v-if="isExistsStorage"
                  @mouseenter="hoveredRowEmployeeIndex = index"
                  @mouseleave="hoveredRowEmployeeIndex = null"
                  @click="employeeLineMarking(item)"
                  :class="{
                    'bg-grey-lighten-2': markedEmployeeID.includes(item.id),
                  }"
      ...
      <v-dialog persistent v-model="dataDialog" activator="parent">
        ...
        >
          <div class="d-flex justify-space-between align-center pr-5 pt-3">
            <span class="pl-5"
              >{{
                isExistsStorageData ? "Изменить" : "Добавить"
              }}
              сотрудника</span
            >
            <div class="d-flex align-center justify-space-between">
              <div class="d-flex ga-3 align-center mt-2">
                <Icons
                  v-if="isExistsStorageData"
                  @click="removeStorageEmployee"
                  name="delete"
                />
                <Icons
                  v-if="isExistsStorageData"
