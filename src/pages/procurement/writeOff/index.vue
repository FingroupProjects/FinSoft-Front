...
const getProcurementData = async ({
  ...
  loading.value = true;
  try {
    const { data } = await writeOffApi.get(
...
const massDel = async () => {
  try {
    const { status } = await writeOffApi.remove({
      ids: markedID.value,
    });
    if (status === 200) {
...
const massRestore = async () => {
  try {
    const { status } = await writeOffApi.restore({ ids: markedID.value });
...
const approve = async () => {
  if (markedID.value.length === 0) return showToast(warningMessage, "warning");
  try {
    const res = await writeOffApi.approve({ ids: markedID.value });
...
const unApprove = async () => {
  if (markedID.value.length === 0) return showToast(warningMessage, "warning");
  try {
    await writeOffApi.unApprove({ ids: markedID.value });
    showToast(approveDocument);
    await getProcurementData({});
    markedID.value = [];
  } catch (e) {
  ...
  <div class="pa-4">
    <div class="d-flex justify-space-between calcWidth">
      <div class="d-flex align-center ga-2 pe-2 ms-4">
        <span :style="{ color: TITLE_COLOR, fontSize: '22px' }">Списание</span>
      </div>
