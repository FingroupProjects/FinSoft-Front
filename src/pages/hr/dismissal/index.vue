...
const headerButtons = ref([
  {
    name: "create",
    function: () => router.push({ name: "dismissalCreate" }),
  },
  {
    name: "createBasedOn",
  },
...
const getDismissalData = async ({
  ...
  loading.value = true;
  try {
    const { data } = await dismissal.get(
...
const lineMarking = (item) => {
  if (markedID.value.length > 0) {
    const firstMarkedItem = dismissals.value.find(
