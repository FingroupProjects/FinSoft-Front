...
const getDismissalData = async ({page, itemsPerPage, sortBy, search}) => {
  count.value = 0
  countFilter()
...
const lineMarking = (item) => {
  if (markedID.value.length > 0) {
    const firstMarkedItem = dismissals.value.find(el => el.id === markedID.value[0]);
    ...
    <v-col>
      <div class="d-flex justify-space-between text-uppercase ">
        <div class="d-flex align-center ga-2 pe-2 ms-4">
          <span>Увольнение кадра</span>
        </div>
        <v-card variant="text" min-width="350" class="d-flex align-center ga-2">
          <div class="d-flex w-100">
            <div class="d-flex ga-2 mt-1 me-3">
              <Icons title="Добавить" @click="$router.push('/hr/dismissalCreate')" name="add"/>
      ...
      <v-card class="mt-2 table">
        <v-data-table-server
            ...
            v-model="markedID"
            @update:options="getDismissalData"
            page-text =  '{0}-{1} от {2}'
